    ...
    F: Fn(&T, &T) -> T,
{
    pub fn from_vec(elements: &[T], sum: F) -> Self {
        ...
        for (i, element) in elements.iter().enumerate() {
            data[size + i] = element.clone();
        }

        for i in (1..size).rev() {
            data[i] = sum(&data[2 * i], &data[2 * i + 1])
        }
    ...
    pub fn update(&mut self, index: usize, value: T) -> Result<(), &str> {
        ...
        while node > 0 {
            node /= 2;
            self.data[node] = (self.f)(
                self.data.get(2 * node).unwrap(),
                self.data.get(2 * node + 1).unwrap(),
            )
        }
        ...
        R: RangeBounds<usize>,
    {
        ...
        while left < right {
            if left % 2 == 1 {
                result = (self.f)(&result, self.data.get(left).unwrap());
                left += 1;
            }
            if right % 2 == 1 {
                right -= 1;
                result = (self.f)(&result, self.data.get(right).unwrap());
            }
            left /= 2;
            right /= 2;
        }
